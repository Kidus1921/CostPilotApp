...
import { BellIcon, SunIcon, MoonIcon, InformationCircleIcon } from './IconComponents';
import NotificationPanel from './notifications/NotificationPanel';
import Avatar from './Avatar';
import { useAppContext } from '../AppContext';
import SqlInfoModal from './SqlInfoModal';
    ...
    <>
      <header className="sticky top-0 z-[50] flex items-center justify-between h-20 px-4 sm:px-6 bg-base-100 border-b border-base-300 dark:bg-[#111111] dark:border-white/10 shadow-sm">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          {/* System SQL Info Button - Restricted to Admin */}
          {currentUser.role === UserRole.Admin && (
            <button 
              onClick={() => setIsSqlModalOpen(true)} 
              className="p-2 rounded-full hover:bg-base-200 dark:hover:bg-white/5 transition-colors" 
              title="System Control SQL"
            >
              <InformationCircleIcon className="w-6 h-6 text-gray-400 hover:text-brand-primary" />
            </button>
          )}
      ...
      <SqlInfoModal isOpen={isSqlModalOpen} onClose={() => setIsSqlModalOpen(false)} />
    </>
  );
};

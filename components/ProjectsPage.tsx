...
import { ArrowLeftIcon, TrashIcon, PauseIcon, PlayIcon, CheckCircleIcon, PaperclipIcon, PhotographIcon, FilePdfIcon, FileExcelIcon, DocumentTextIcon, PlusIcon } from './IconComponents';
...
const seedDatabase = async () => {
    ...
    if (snapshot.empty) {
        ...
        const mockProjects: Omit<Project, 'id'>[] = [
            ...
            { title: 'Mobile App "ConnectU"', description: 'A new social networking app for professionals.', startDate: '2023-06-10', endDate: '2024-12-20', teamLeader: mockUsers[2], team: [mockUsers[2], mockUsers[0]], status: ProjectStatus.Pending, completionPercentage: 0, budget: 80000, spent: 0, tasks: [], expenses: [], documents: [] },
            { title: 'Data Center Migration', description: 'Migrating all servers to a new cloud infrastructure.', startDate: '2022-11-01', endDate: '2024-05-20', teamLeader: mockUsers[0], team: mockUsers.slice(0,2), status: ProjectStatus.Completed, completionPercentage: 100, budget: 300000, spent: 285000, tasks: [], expenses: [], documents: [] },
            { title: 'Internal HR Portal', description: 'A new portal for employee management.', startDate: '2023-09-01', endDate: '2024-09-30', teamLeader: mockUsers[1], team: [mockUsers[1]], status: ProjectStatus.OnHold, completionPercentage: 0, budget: 45000, spent: 0, tasks: [], expenses: [], documents: [] },
        ];
...
// TASK LIST COMPONENT
// ===================================================================================
const TaskList: FC<{ project: Project; onUpdateTask: (updatedTask: Task) => void; onAddTask: (taskName: string) => void; onDeleteTask: (taskId: string, taskName: string) => void; }> = ({ project, onUpdateTask, onAddTask, onDeleteTask }) => {
        ...
        <div className="bg-base-100 p-4 rounded-lg shadow-sm dark:bg-gray-800">
            ...
            {!isProjectPending && (
                 <form onSubmit={handleAddTaskSubmit} className="mt-4 flex gap-2">
                    ...
                    />
                    <button type="submit" className="bg-brand-primary text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-teal-700">Add Task</button>
                </form>
            )}
...
const DocumentsList: FC<{ project: Project; onUpdateProject: (data: Partial<Project>) => void; }> = ({ project, onUpdateProject }) => {
    ...
    return (
        <div className="bg-base-100 p-4 rounded-lg shadow-sm dark:bg-gray-800">
            <div className="flex justify-between items-center mb-4">
                ...
                <label htmlFor="file-upload" className="bg-brand-primary text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-teal-700 transition-colors cursor-pointer flex items-center gap-2">
...
// ===================================================================================
const ProjectDetailPage: FC<{ project: Project; onBack: () => void; }> = ({ project, onBack }) => {
    ...
    const handleApproveProject = () => {
        ...
        updateProjectInDb({ status: ProjectStatus.InProgress });
    };
        ...
        <div className="space-y-6">
                     ...
                     <div className="flex flex-col items-end gap-4">
                        ...
                        {project.status === ProjectStatus.Pending && (
                            <button 
                                ...
                                className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-green-600 transition-colors flex items-center gap-2"
                            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <TaskList project={project} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} onDeleteTask={handleDeleteTask}/>
                    <DocumentsList project={project} onUpdateProject={updateProjectInDb} />
                </div>
                <div className="bg-base-100 p-6 rounded-xl shadow-md space-y-4 dark:bg-gray-800">
                    ...
                    <div><p className="font-semibold dark:text-gray-200">Dates</p><p className="dark:text-gray-300">{new Date(project.startDate).toLocaleDateString()} - {new Date(project.endDate).toLocaleDateString()}</p></div>
                    <div>
...
const ProjectsPage: React.FC = () => {
    ...
    const handleAddProject = async (newProjectData: NewProjectData) => {
        ...
        try {
            ...
            // Notify admins of new project for approval
            ...
            adminSnapshot.forEach(adminDoc => {
                createNotification({
                    ...
                    priority: NotificationPriority.High,
                    link: `/financials` // Link to approvals tab
                });
            });

...
import { 
    ...
    ChatIcon, XCircleIcon, RefreshIcon
} from '../IconComponents';
...
const ProjectsPage: React.FC = () => {
    ...
    if (selectedProject) {
        ...
        const isRejected = selectedProject.status === ProjectStatus.Rejected;
        ...
        return (
            <div className="space-y-6 animate-fadeIn pb-20">
                ...
                {/* Rejection Notice */}
                {isRejected && (
                    <div className="bg-brand-tertiary/10 border border-brand-tertiary/30 p-6 rounded-2xl flex items-start gap-4 animate-fadeIn ring-4 ring-brand-tertiary/5">
                        <div className="p-3 bg-brand-tertiary/20 rounded-xl">
                            <XCircleIcon className="w-6 h-6 text-brand-tertiary" />
                        </div>
                        <div>
                            <h3 className="text-xs font-black text-brand-tertiary uppercase tracking-[0.2em] mb-1">Operational Rejection Notice</h3>
                            <p className="text-sm font-bold text-gray-800 dark:text-gray-200">
                                {selectedProject.rejectionReason || "No specific reason provided by the authority lead."}
                            </p>
                            <p className="text-[10px] text-brand-tertiary/60 uppercase font-bold mt-2 tracking-widest italic">Action Required: Revise project scope and resubmit.</p>
                        </div>
                    </div>
                )}
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-base-100 dark:bg-[#111111] p-6 rounded-2xl shadow-sm border border-base-300 dark:border-white/10">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                {canManageUploads && !isOnHold && !isRejected && (
                                    <>
                                                        ...
                                                        {isEditing ? (
                                                            ...
                                                            <>
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {canModify && !isOnHold && !isCompleted && !isRejected && (
                ...
                {isEditModalOpen && (
                    <CreateProjectModal 
                        ...
                        onSave={async (data) => {
                            await persistProjectUpdates({ ...data, status: ProjectStatus.Pending, rejectionReason: null }, 'Scope Revision', selectedProject.title);
                            setIsEditModalOpen(false);
                            setSelectedProject(null); // Return to list as it's now pending again
                        }} 
                    />
                )}
            </div>

...
const NotificationSettingsTab: React.FC<NotificationSettingsTabProps> = ({ currentUser }) => {
    ...
    if (loading) return <div className="p-20 text-center animate-pulse uppercase tracking-[0.3em] font-bold text-gray-500">Syncing Registry...</div>;
    if (!preferences) return null;

    return (
        <div className="space-y-6 max-w-4xl mx-auto animate-fadeIn">
            <div className="bg-base-100 dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-base-300 dark:border-gray-700">
                <h3 className="text-xl font-bold text-base-content dark:text-white mb-8 uppercase tracking-widest">Alert Protocols</h3>
                
                <div className="bg-brand-primary/5 dark:bg-brand-primary/10 rounded-2xl p-6 mb-8 flex items-center justify-between border border-brand-primary/10">
                    <div>
                     ...
                     <button onClick={handlePushToggle} className={`relative inline-flex h-6 w-12 items-center rounded-full transition-all ${preferences.pushEnabled ? 'bg-brand-primary' : 'bg-gray-300 dark:bg-gray-600'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    {['inApp', 'email'].map((section) => (
                        <div key={section}>
                            <h4 className="font-bold text-xs uppercase tracking-[0.2em] mb-6 text-gray-400 border-b border-base-200 pb-2">{section === 'inApp' ? 'In-App Telemetry' : 'Email Relays'}</h4>
                ...
                <div className="flex justify-end pt-10 border-t border-base-200 dark:border-gray-700 mt-8">
